 ...
 * 
 * file: _matches-polyfill.js
 */

//console.log('_matches-polyfill.js file loaded');
...
*/
MW.Util.hasClass = function(el, className) {
	if (el.classList) return el.classList.contains(className);
	else return !!el.className.match(new RegExp('(\\s|^)' + className + '(\\s|$)'));
};

MW.Util.addClass = function(el, className) {
	var classList = className.split(' ');
 	if (el.classList) el.classList.add(classList[0]);
 	else if (!MW.Util.hasClass(el, classList[0])) el.className += " " + classList[0];
 	if (classList.length > 1) MW.Util.addClass(el, classList.slice(1).join(' '));
};

MW.Util.removeClass = function(el, className) {
	var classList = className.split(' ');
	if (el.classList) el.classList.remove(classList[0]);	
	else if (MW.Util.hasClass(el, classList[0])) {
		var reg = new RegExp('(\\s|^)' + classList[0] + '(\\s|$)');
		el.className=el.className.replace(reg, ' ');
	}
	if (classList.length > 1) MW.Util.removeClass(el, classList.slice(1).join(' '));
};

MW.Util.toggleClass = function(el, className, bool) {
	if (bool) Util.addClass(el, className);
	else Util.removeClass(el, className);
};

// Toggle element visibility
MW.Util.toggle = function (elem, classname) {
...
Util.functionExists = function (method, scope) {
	if (scope) {
...
MW.Util.getIndexInArray = function(array, el) {
...
/* 
  Arrays
*/
MW.Util.arrayRemove = function(arr, value) {
...
MW.Util.handlebarsDebugHelper = function() {
